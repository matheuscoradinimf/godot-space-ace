...
func shoot() -> void:
	...
	SignalHub.emit_on_create_bullet(global_position, bullet_direction, bullet_speed, bullet_type)
	sound.play()
	start_shoot_timer()
...
func create_power_up() -> void:
	if randf() < power_up_chance:
		var types = [PowerUp.PowerUpType.HEALTH, PowerUp.PowerUpType.SHIELD]
		var type_chosen = types[randi() % types.size()]
		SignalHub.emit_on_create_power_up(global_position, type_chosen)
